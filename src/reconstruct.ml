...
let reconstruct (type cert)
  ...
  let rec right_focus ?(lev=0) ~succ ~fail sq c =
    assert (List.length sq.left_active = 0) ;
    Format.(
      if __debug then
        fprintf std_formatter "%sright_focus: %a@.  %a@."
          (indent lev)
  ...
  and left_focus ?(lev=0) ~succ ~fail sq c =
    Format.(
      if __debug then
        fprintf std_formatter "%sleft_focus: %a@.  %a@."
          (indent lev)
          format_sequent sq Ag.format_cert c
    ) ;
    let lev = lev + 1 in
  ...
  and right_active ?(lev=0) ~succ ~fail sq c =
    Format.(
      if __debug then
        fprintf std_formatter "%sright_active: %a@.  %a@."
          (indent lev)
          format_sequent sq Ag.format_cert c
    ) ;
    let lev = lev + 1 in
  ...
  and left_active ?(lev=0) ~succ ~fail sq c =
    Format.(
      if __debug then
        fprintf std_formatter "%sleft_active: %a@.  %a@."
          (indent lev)
          format_sequent sq Ag.format_cert c
    ) ;
    let lev = lev + 1 in
  ...
  and frontier ?(lev=0) ~succ ~fail sq c =
    Format.(
      if __debug then
        fprintf std_formatter "%sfrontier: %a@.  %a@."
          (indent lev)
          format_sequent sq Ag.format_cert c ;
    ) ;
    let lev = lev + 1 in
